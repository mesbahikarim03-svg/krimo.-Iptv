...
# ================== [تعديل 1] تحليل الملفات بأقصى سرعة متوازية ==================
def analyze_file_fast(filepath):
    """نسخة مُحسَّنة عالية السرعة: قراءة دفعية + تجميع فوري بدون إدخال/إخراج زائد"""
    ...
    # فلترة مجموعات البالغين من أسماء المجموعات
...
# الدالة القديمة تبقى كـ fallback
...
async def analyze_async(filepath):
    """استخدام النسخة الأسرع تلقائياً"""
    return await asyncio.to_thread(analyze_file_fast, filepath)
...
# ================== [تعديل 1] جلب وتحليل الروابط بأقصى توازٍ ==================
async def fetch_and_analyze(session, url, idx):
    """نسخة مُسرَّعة: timeout أقصر + قراءة دفعية أسرع"""
    async def _fetch():
        try:
            fast_timeout = aiohttp.ClientTimeout(total=15, sock_connect=5, sock_read=10)
            async with session.get(url, headers={"User-Agent": "Mozilla/5.0"}, allow_redirects=True, timeout=fast_timeout) as response:
                if response.status in [200, 206]:
                    temp = f"temp_{uuid.uuid4().hex}.m3u"
                    # قراءة بحجم مخزن مؤقت أكبر لسرعة أعلى
                    with open(temp, 'wb') as f:
                        async for chunk in response.content.iter_chunked(2 * 1024 * 1024):
                            f.write(chunk)
                    groups, total, adult = await analyze_async(temp)
                    safe_delete(temp)
                    if total < MIN_CHANNELS_REQUIRED or not await is_playlist_alive(groups):
                        return {"id": idx, "success": False}
                    return {"id": idx, "groups": groups, "total": total, "size_mb": get_clean_size_mb(groups), "success": True}
        except: pass
        return {"id": idx, "success": False}
    try:
        return await asyncio.wait_for(_fetch(), timeout=18.0)
    except:
        return {"id": idx, "success": False}
...
# ================== [تعديل 2] توليد صورة احترافية بالذكاء الاصطناعي ==================
def build_image_prompt(keyword, num_links, is_hunter=True):
    """بناء prompt احترافي لتوليد صورة IPTV مناسبة للمنشور"""
    base = "Professional digital banner for an IPTV streaming service"
    if keyword:
        kw_upper = keyword.upper()
        prompt = (
            f"{base}, featuring glowing text '{kw_upper} IPTV SERVER', "
            f"dark background with neon blue and gold light streaks, "
            f"satellite dish, streaming icons, world map, cinematic 4K quality, "
            f"futuristic tech aesthetic, NO text overlays except the title"
        )
    else:
        prompt = (
            f"{base}, showing global satellite network, streaming symbols, "
            f"neon blue and purple glowing interface on dark background, "
            f"premium VIP feel, world flags icons, cinematic 4K banner style"
        )
    return prompt

async def generate_post_image_pollinations(prompt):
    """
    توليد صورة مجانية عبر Pollinations AI (لا يحتاج API key).
    يعيد bytes الصورة أو None عند الفشل.
    """
    try:
        encoded = requests.utils.quote(prompt)
        # نستخدم seed عشوائي لتنويع الصور
...
def build_promo_caption(keyword, num_links, cap_title):
    """
    قالب بسيط جداً احترافي للمنشور مع الصورة (بدون روابط).
    """
    now = datetime.utcnow().strftime("%Y-%m-%d %H:%M UTC")
    if keyword:
        subject = f"🎯 <b>{keyword.upper()} IPTV</b>"
    else:
        subject = "🌍 <b>FREE IPTV WORLD</b>"

    return (
        f"🔥 {cap_title} 🔥\n\n"
        f"{subject}\n\n"
        f"📦 <b>عدد الباقات:</b> {num_links} سيرفر\n"
        f"⚡ <b>الجودة:</b> Premium HD / FHD\n"
        f"🕒 <b>وقت النشر:</b> {now}\n\n"
        f"👇 <b>الروابط المباشرة في المنشور التالي أدناه 👇</b>\n\n"
        f"📣 @free_iptv_world"
    )

async def publish_with_ai_image(bot, keyword, num_links, cap_title, links_text, full_caption):
    """
    ينشر أولاً صورة + قالب بسيط (بدون روابط)،
    ثم ينشر المنشور الأصلي الكامل بالروابط كما هو.
    """
    # --- توليد الصورة ---
    prompt = build_image_prompt(keyword, num_links)
    image_bytes = await generate_post_image_pollinations(prompt)

    promo_cap = build_promo_caption(keyword, num_links, cap_title)

    if image_bytes:
        # نشر الصورة مع القالب البسيط أولاً
        img_file = f"promo_{uuid.uuid4().hex[:6]}.jpg"
        try:
            ...
            # fallback: نشر كنص إن فشل إرسال الصورة
            ...
            safe_delete(img_file)
    else:
        # إن فشل التوليد: نشر القالب النصي البسيط فقط
        await bot.send_message(
            chat_id=CHANNEL_ID,
            text=promo_cap,
            parse_mode="HTML",
    ...
    # انتظار ثانية واحدة ثم نشر المنشور الأصلي كاملاً بالروابط
...
async def run_hunter_action(bot, chat_id, message_id, args):
    try:
        ...
        if collected_links:
            ...
            full_caption = WARNING_TEXT + LINK_POST_CAPTION.replace(
                "🔗 𝗗𝗜𝗥𝗘𝗖𝗧 𝗜政𝗧𝗩 𝗟𝗜𝗡𝗞𝗦 🔗\n🌍 𝗙𝗥𝗘𝗘 𝗜𝗣𝗧𝗩 𝗪𝗢𝗥𝗟𝗗 🌍",
                f"🔗 {cap_title} 🔗\n🌍 𝗙𝗥𝗘𝗘 𝗜𝗣𝗧𝗩 𝗪𝗢𝗥𝗟𝗗 🌍"
            ).replace("{links}", "\n\n".join(collected_links))

            # [تعديل 2] نشر الصورة الاحترافية أولاً ثم المنشور الأصلي
            await publish_with_ai_image(bot, keyword, found_count, cap_title, "\n\n".join(collected_links), full_caption)
