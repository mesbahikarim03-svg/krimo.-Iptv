...
LINK_POST_CAPTION = """🔗 𝗗𝗜𝗥𝗘𝗖𝗧 𝗜𝗣𝗧𝗩 𝗟Ｉ𝗡𝗞𝗦 🔗
🌍 𝗙𝗥𝗘𝗘 𝗜𝗣𝗧𝗩 𝗪𝗢𝗥𝗟𝗗 🌍
...
async def run_hunter_action(bot, chat_id, message_id, args):
    try:
        ...
        async with aiohttp.ClientSession() as session_req:
            async for dialog in app.get_dialogs():
                ...
                if any(kw in chat_name.lower() for kw in TARGET_KEYWORDS):
                    ...
                    urls_to_test = set()
                    try:
                        # جلب 150 رسالة لحل مشكلة نقص العدد تماماً
                        async with asyncio.timeout(15):
                            async for msg in app.get_chat_history(chat.id, limit=150):
                                ...
                                if host and user and pw: urls_to_test.add(f"{host.group(1)}/get.php?username={user.group(1)}&password={pw.group(1)}&type=m3u_plus&output=ts")
                    except: pass
        ...
        if collected_links:
            await safe_edit(bot, chat_id, message_id, f"🚀 **انتهى الصيد!**\nجاري النشر في القناة بالترتيب الجديد...", edit_state, None, force=True)
            
            # --- الخطوة 1: إرسال الصورة الفخمة كمنشور مستقل بقالب بسيط وبدون أي روابط ---
            ai_image_url = generate_ai_image_url(keyword)
            simple_img_caption = f"🏆 <b>𝗙𝗥𝗘𝗘 𝗜𝗣𝗧𝗩 <b>𝗪𝗢𝗥𝗟𝗗</b></b> 🏆\n⚡ <i>New Exclusive {keyword.upper() if keyword else 'Premium'} Package Uploaded!</i>\n\n👇 <b>Check the links in the messages below</b> 👇"
            
            try:
                await bot.send_photo(chat_id=CHANNEL_ID, photo=ai_image_url, caption=simple_img_caption, parse_mode="HTML", reply_markup=build_post_keyboard())
                await asyncio.sleep(2)
            except: pass
            
            # --- الخطوة 2: إرسال المنشورات الأصلية والقوالب العادية تحتها مباشرة ---
            ...
            await bot.send_message(chat_id=CHANNEL_ID, text=caption_1, parse_mode="HTML", disable_web_page_preview=True, reply_markup=build_post_keyboard())
            await asyncio.sleep(3)
            
            # نشر باقي الدفعات (كل 10 روابط) في رسائل مستقلة
            if remaining_links:
                chunk_size = 10
                for i in range(0, len(remaining_links), chunk_size):
                    chunk = remaining_links[i:i + chunk_size]
