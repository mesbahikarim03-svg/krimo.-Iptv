...
# ================== أوامر الصيد والسحب ==================
async def run_hunter_action(bot, chat_id, message_id, args, photo_id):
    try:
        edit_state = {"time": 0}
        ...
        if collected_links:
            ...
            # --- الخطوة 1: نشر الصورة الفخمة من السيرفر مباشرة عبر الـ photo_id ---
            ...
            try:
                if photo_id:
                    await bot.send_photo(chat_id=CHANNEL_ID, photo=photo_id, caption=simple_img_caption, parse_mode="HTML")
                else:
                    # صورة بديلة إذا لم يرسل الاستضافة الرمز
                    await bot.send_photo(chat_id=CHANNEL_ID, photo="https://files.catbox.moe/goe4nn.jpg", caption=simple_img_caption, parse_mode="HTML")
...
async def main():
    ...
    photo_id = payload.get("photo_id")  # الرمز السري للصورة يجي من الاستضافة مباشرة!
    ...
    try:
        if action == "hunt":
            await run_hunter_action(bot, chat_id, message_id, args, photo_id)
        elif action == "hunttxt":
            await run_hunttxt_action(bot, chat_id, message_id, args)
        elif action == "scrape":
            await run_scrape_action(bot, chat_id, message_id, args)
